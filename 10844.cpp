...
bigint	*answer = new bigint [ MaxI + 1 ] ;
bigint	*bellTriangle_i = new bigint [ MaxI + 1 ] ;
bigint	*bellTriangle_i_1 = new bigint [ MaxI + 1 ] ;
...
main()
{
	...
	for( i = 2 ; i <= MaxI ; i++ )
	{
		swap( bellTriangle_i, bellTriangle_i_1 ) ;

		bellTriangle_i[ 1 ] = bellTriangle_i_1[ i - 1 ] ;
		for( j = 2 ; j <= i ; j++ )
		{
			bellTriangle_i[ j ] = bellTriangle_i[ j - 1 ] + bellTriangle_i_1[ j - 1 ] ;
		}
		answer[ i ] = bellTriangle_i[ i ] ;
	}
