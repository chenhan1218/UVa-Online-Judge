...
main()
{
	...
	for( n = 2 ; n <= MaxN ; n += 2 )
	{
		...
		for( d = 2 ; d <= MaxD ; d++ )
		{
			...
			for( i = 2 ; i <= n - d * 2 ; i += 2 )
			{
					expression[ n ][ d ] += expressionLEtoD[ i - 2 ][ d - 2 ] * 
											expression[ n - i ][ d ] ;
			}
			for( i = d * 2 ; i <= n ; i += 2 )
			{
					expression[ n ][ d ] += expression[ i - 2 ][ d - 1 ] * 
											expressionLEtoD[ n - i ][ d ] ;
			}

			// calculate expressionLEtoD[ n ][ d ]
			expressionLEtoD[ n ][ d ] = 	expressionLEtoD[ n ][ d - 1 ] + 
											expression[ n ][ d ] ;
		}
	}
